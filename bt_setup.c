...
void parse_args(bt_args_t * bt_args, int argc,  char * argv[]){
  ...
  bt_args->ipFlag = 0;
  bt_args->pFlag = 0;
  bt_args->saveFileFlag = 0;

  /* set the default args */
  bt_args->verbose=0; // no verbosity
  bt_args->port = 0; // port flag
  ...
  while ((ch = getopt(argc, argv, "hp:b:s:l:vI:")) != -1) {
    switch (ch) {
    case 'h': //help
      usage(stdout);
      exit(0);
    ...
    case 'I':
      //bt_args->id = atoi(optarg);
    	...
    	fprintf(stderr, "Inputting custom ID is not allowed\n");
      break;
    default:
